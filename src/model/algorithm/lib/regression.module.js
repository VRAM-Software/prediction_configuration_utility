class Regression {
    ...
    constructor(options) {
        ...
        this.transposeOfXTimesX = this.rectMatrix({
            numRows: options.numX,
            numColumns: options.numX,
        }) //matrice
        this.transposeOfXTimesY = this.rectMatrix({
            numRows: options.numX,
    ...
    addObservation(options) {
        if (!options) {
            ...
            !(options.x instanceof Array) ||
            !(options.y instanceof Array)
        ) {
            throw new Error("x and y must be given as arrays")
        }
        this.addRowAndColumn(this.transposeOfXTimesX, {
            lhsColumn: options.x,
            rhsRow: options.x,
        })
        this.addRowAndColumn(this.transposeOfXTimesY, {
            lhsColumn: options.x,
            rhsRow: options.y,
        })
    ...
    hypothesize(options) {
        ...
        for (let x = 0; x < this.coefficients.length; x++) {
            ...
            for (let y = 0; y < coefficientRow.length; y++) {
                hypothesis[y] =
                    (hypothesis[y] || 0) + coefficientRow[y] * options.x[x]
            }
        }
        return hypothesis
    }
